...
router.get("/site/:id", async (req, res) => {
  ...
  try {
    const [rows] = await connection.execute("SELECT * FROM t_lieu WHERE lieu_id = ?", [siteId]);
    if (rows.length === 0) {
...
router.post("/site/:id/visiter", async (req, res) => {
  ...
  const connection = await mysql.createConnection(dbConfig);
  try {
    await connection.execute("INSERT INTO t_visiter (lieu_id_fk, compte_id_fk) VALUES (?, ?)", [siteId, compteId]);
    res.json({ message: `Site avec l'ID ${siteId} ajouté à la liste "à visiter"` });
  } catch (error) {
    res.status(500).json({ message: "Erreur lors de l'ajout à la liste.", error });
  } finally {
    await connection.end();
  }
});
...
router.post("/site/:id/visite", async (req, res) => {
  ...
  const connection = await mysql.createConnection(dbConfig);
  try {
    await connection.execute("INSERT INTO t_visiter (lieu_id_fk, compte_id_fk, date_visite) VALUES (?, ?, ?)", [siteId, compteId, dateVisite]);
    res.json({ message: `Site avec l'ID ${siteId} ajouté à la liste "visité"` });
  } catch (error) {
    res.status(500).json({ message: "Erreur lors de l'ajout à la liste.", error });
  } finally {
    await connection.end();
...
router.delete("/site/:id/visite", async (req, res) => {
  ...
  const connection = await mysql.createConnection(dbConfig);
  try {
    await connection.execute("DELETE FROM t_visiter WHERE lieu_id_fk = ? AND compte_id_fk = ?", [siteId, compteId]);
    res.json({ message: `Site avec l'ID ${siteId} enlevé de la liste "visité"` });
  } catch (error) {
...
router.delete("/site/:id/a-visiter", async (req , res) => {
  ...
  const connection = await mysql.createConnection(dbConfig);
  try {
    await connection.execute("DELETE FROM t_aimeraitVisiter WHERE lieu_id_fk = ? AND compte_id_fk = ?", [siteId, compteId]);
    res.json({ message: `Site avec l'ID ${siteId} enlevé de la liste "à visiter"` });
  } catch (error) {
    res.status(500).json({ message: "Erreur lors de la suppression de la liste.", error });
  } finally {
    await connection.end();
...
router.post("/site", isAdmin, async (req, res) => {
  ...
  const connection = await mysql.createConnection(dbConfig);
  try {
    const [result] = await connection.execute("INSERT INTO t_lieu (nom, longitude, latitude, particularite) VALUES (?, ?, ?, ?)", [nom, longitude, latitude, particularite]);
    res.status(201).json({ message: `Site ajouté avec l'ID ${result.insertId}` });
  } catch (error) {
...
router.patch("/site/:id", isAdmin, async (req, res) => {
  ...
  const connection = await mysql.createConnection(dbConfig);
  try {
    await connection.execute("UPDATE t_lieu SET nom = ?, longitude = ?, latitude = ?, particularite = ? WHERE lieu_id = ?", [nom, longitude, latitude, particularite, siteId]);
    res.json({ message: `Informations du site avec l'ID ${siteId} modifiées` });
  } catch (error) {
    res.status(500).json({ message: "Erreur lors de la modification des informations.", error });
  } finally {
    await connection.end();
  }
});
...
router.post("/site/:id/comment", async (req, res) => {
  ...
  const connection = await mysql.createConnection(dbConfig);
  try {
    await connection.execute("INSERT INTO t_avis (commentaire, rating, lieu_id_fk, compte_id_fk) VALUES (?, ?, ?, ?)", [commentaire, rating, siteId, compte_id]);
...
router.patch("/site/:id/comment/:commentId", isAdmin, async (req, res) => {
  ...
  const connection = await mysql.createConnection(dbConfig);
  try {
    await connection.execute("UPDATE t_avis SET commentaire = ?, rating = ? WHERE avis_id = ? AND lieu_id_fk = ?", [commentaire, rating, commentId, siteId]);
    res.json({ message: `Commentaire avec l'ID ${commentId} modifié pour le site avec l'ID ${siteId}` });
  } catch (error) {
...
router.delete("/site/:id/comment/:commentId", isAdmin, async (req, res) => {
  ...
  const connection = await mysql.createConnection(dbConfig);
  try {
    await connection.execute("DELETE FROM t_avis WHERE avis_id = ? AND lieu_id_fk = ?", [commentId, siteId]);
    res.json({ message: `Commentaire avec l'ID ${commentId} supprimé pour le site avec l'ID ${siteId}` });
  } catch (error) {
